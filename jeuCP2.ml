...
let draw_absolute_pt(p, base_draw, dilat, col : t_point * t_point * int * t_color) : unit =
  let (p_dilat_x, p_dilat_y) : int * int = ((p.x * dilat) + base_draw.x, (p.y * dilat) + base_draw.y) in (**point multiplié par coefficient d'homothétie puis total ajouté au point base (nous faisons donc une translation).*)
  (
  set_color(col) ;
  draw_rect(p_dilat_x, p_dilat_y, dilat - 1, dilat - 1) ; (**puis nous faisons appel au fonctions graphique ocaml. Contour rectangle de longueur et largeur dilat - 1, de couleur col.*) 
  )
;;
...
let drawfill_absolute_pt(p,  base_draw, dilat, col : t_point * t_point * int * t_color) : unit =
  ...
  draw_absolute_pt(p, base_draw, dilat, black) (**appel aux fonctions précèdentes avec couleur du contour en noir et rempli de couleur col.*)
  )
;;
...
let rec draw_pt_list(l, base_pt, base_draw, dilat, col : t_point list * t_point * t_point * int * t_color) : unit =
  if isempty(l) 
  then () (**première conséquence lorsque conditionnel list l vide.*)
  else
    (
      draw_relative_pt(fst(l), base_pt, base_draw, dilat, col) ; (**Alternance avec appel de la fonction draw_relative_pt()*)
    draw_pt_list(rem_fst(l), base_pt, base_draw, dilat, col) (**puis de la fonction récursive avec list où le premier élément est retiré.*)
    )
;;
...
(*draw_frame*)
(**Trois boucles itératives, tous les boucles font appel à la fonction drawfill_absolute_pt() avec couleur noir*)
let draw_frame(base_draw, size_x, size_y, dilat : t_point * int * int * int) : unit =
  (
    for i = -1 to  size_y - 1 (**première pour le côté gauche du cadre*)
    do
       drawfill_absolute_pt({x = -1 ; y = i}, {x = base_draw.x ; y = base_draw.y}, dilat, black) 
    done
  ) ;
  (
    for a = -1 to size_x - 1 (**deuxième pour le bas*)
    do
       drawfill_absolute_pt({x = a ; y = -1}, {x = base_draw.x ; y = base_draw.y}, dilat, black)
    done
  ) ;
  (
    for i = -1 to size_y - 1 (**troisième pour côté droit*)
    do
